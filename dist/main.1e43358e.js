...
function (_Phaser$Scene) {
  ...
  _createClass(Level01, [{
    key: "preload",
    value: function preload() {
      this.load.spritesheet('player', 'assets/sprite/player.png', {
        frameWidth: 16,
        frameHeight: 28
    ...
    value: function create() {
      // Create world bounds
      this.physics.world.setBounds(0, 0, 800, 600); // Add background, player, and reticle sprites
      // var background = this.add.image(800, 600, 'background');

      this.player = this.physics.add.sprite(400, 300, 'player').setScale(1); // reticle = this.physics.add.sprite(800, 700, 'target');
      // Set image/sprite properties
      // background.setOrigin(0.5, 0.5).setDisplaySize(800, 600);

      this.player.setOrigin(0.5, 0.5).setDisplaySize(100, 120).setCollideWorldBounds(true).setDrag(500, 500); // reticle.setOrigin(0.5, 0.5).setDisplaySize(25, 25).setCollideWorldBounds(true);

      window.player = this.player; //set smaller hitbox

      this.player.setSize(20, 34).setOffset(-3, -3);
      this.player.setCollideWorldBounds(true);
      this.keyboard = this.input.keyboard.addKeys("W, A, S, D"); // Set camera zoom
